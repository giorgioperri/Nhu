...
class Player extends Phaser.Physics.Arcade.Sprite {
	...
	update() {
		...
		const { left, right, space, up, esc } = this.cursors;
		...
		if (
			(isSpaceJustDown || isWJustDown || isUpJustDown) &&
	...
	handleAttacks() {
		this.scene.input.keyboard.on('keydown-K', () => {
			if (this.scene.score <= 0) return;
			if (this.projectiles.fireProjectile(this, 'greenProjectile')) {
				this.play('throw', true);
				this.projectileSound.play();
				this.scene.useBattery();
			}
		});
